...
class MockHistoryService extends HistoryService {
  ...
  Future<bool> addHistory({
    ...
    calls.add({'vid': vid, 'part': part, 'time': time, 'duration': duration});
    return true;
  }
...
void main() {
  ...
  group('onVideoEnded', () {
    test('duration 为 0 时返回 false', () {
      tracker.lock(1, 1);
      final result = tracker.onVideoEnded(1, 1, null);
      expect(result, false);
    });
    ...
    test('使用锁定的 vid/part 而非传入参数', () {
      ...
      // 传入不同的 vid/part，但应使用锁定值
      tracker.onVideoEnded(99, 99, null);
  ...
  group('saveBeforeSwitch', () {
    test('有进度时上报当前位置', () async {
      ...
      await tracker.saveBeforeSwitch(1, 1);
